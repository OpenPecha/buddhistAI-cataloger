...
const MappingSidebar = () => {
  ...
  >({
    ...
    onSuccess: () => {
      ...
      toast.success("alignmentSavedSuccess");
  ...
  >({
    ...
    onError: (error) => {
      ...
      setSaveError(
        error.message ||
          t("mapping.failedToUpdateAlignment") ||
    ...
    if (hasAlignment) {
      ...
      const alignmentAnnotation = spans.alignment_annotation
        .filter(
          (item) =>
            ...
            item.index !== null &&
            item.alignment_index !== undefined
        )
        .map((item) => ({
          span: item.span,
          index:
            typeof item.index === "string"
              ? Number.parseInt(item.index, 10)
              : item.index ?? 0,
          alignment_index: (item.alignment_index ?? []).map((idx) =>
            typeof idx === "string" ? Number.parseInt(idx, 10) : idx
          ),
        }));
      ...
      function filterAlignedArrays(arr1, arr2) {
        ...
        const filteredArr1 = arr1.filter((item) =>
          alignmentIndicesSet.has(item.index)
        );
      ...
      const filtered = filterAlignedArrays(
        createTargetAnnotation,
        createAlignmentAnnotation
      );
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-red-700">
                <strong>WARNING:</strong> This action will permanently modify
